...
int main( void ){	
   ...
   // auto p_gen = melody_generator();
   
   // p_gen.open();
   //    if( 0 ){ auto fe = fur_elise(); fe.play( p_gen );}      
   //    if( 1 ){ rtttl_play( p_gen, sos ); }   
   //    if( 0 ){ rtttl_play( p_gen, let_it_be );  }
   //    if( 0 ){ rtttl_play( p_gen, muppets );  }
   //    if( 0 ){ rtttl_play( p_gen, rickroll );  }
   //    if( 0 ){ rtttl_play( p_gen, one );  }
   // p_gen.close();

   if( 1 ){ auto me = melody(); me.play ( p ); }
}
